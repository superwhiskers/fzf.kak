...
declare-option -docstring "command to provide list of files in jj tree to fzf. Arguments are supported
Supported tools:
    <package>: <value>:
    Git:       ""jj""
...
define-command -hidden fzf-jj %{ evaluate-commands %sh{
    ...
    printf "%s\n" "fzf -kak-cmd %{edit -existing} -items-cmd %{$cmd} -fzf-args %{-m --expect ${kak_opt_fzf_window_map:-ctrl-w} $additional_flags} -filter %{perl -pe \"if (/${kak_opt_fzf_window_map:-ctrl-w}|${kak_opt_fzf_vertical_map:-ctrl-v}|${kak_opt_fzf_horizontal_map:-ctrl-s}|^$/) {} else {print \\\"$(jj root)/\\\"}\"}"
}}
